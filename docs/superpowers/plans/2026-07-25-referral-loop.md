...
>
> 6. **The event insert and the projection write share one transaction on one connection.** Committing them separately left a crash window where a brand-new loop was durable in the event log but absent from `loops` — and `open_loops()` reads ids from `loops`, so the loop was invisible on the coordinator worklist. `_materialize(loop_id, conn)` and `_replay_on(conn, loop_id)` now run inside the caller's transaction so it sees its own uncommitted event. `__init__` reconciliation was rejected as the fallback: a listener that loses a worker thread but keeps the process up would never restart, and therefore never reconcile.
> 7. **Every store failure is typed.** `_connect()` runs the PRAGMAs, which is where SQLite actually opens the file, so it was leaking raw `sqlite3.OperationalError` on disk-full / unmounted-volume / permission-denied — bypassing the listener's `StoreUnavailableError` handler and therefore the `AE` path those failures exist for. Wrapping inside `_connect` types all call sites at once.
> 8. **`resulted_unacknowledged()` added.** `reopened` maps to `RESULTED`, which `open_loops()` does not select, so a loop whose acknowledgement was just cleared by safety rule 2 — the population most needing a human — was returned by nothing but unfiltered `all_loops()`. **Task 11's worklist must query both.**
>
> **On the durability tests, precisely:** `test_durability_survives_a_hard_kill` is genuinely out-of-process and proves durability against *process* death — the crash-during-parse case persist-before-ACK exists for. It still passes under `synchronous=OFF`, because a committed write sits in the OS page cache and killing a process cannot lose it. Only power loss can, and no in-process test simulates that; `test_durability_pragmas_are_pinned_not_defaulted` is the sole guard there. Neither test subsumes the other — keep both.
>
