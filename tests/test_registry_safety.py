...
# ---------------------------------------------------------------- SPEC TESTS END
# Adversarial probes. Each is a route to CLOSED, or to a loop vanishing from
# every worklist query, that the six tests above do not close.


def test_a_preliminary_arriving_after_a_final_still_blocks_close(registry):
    """Radiology resends a prelim after the final. The newest read we hold is
    preliminary, so CLOSED must be unreachable until a final or correction."""
...
def test_a_preliminary_after_a_correction_blocks_close(registry):
    """Correction reopens, then a prelim lands. Still not closeable."""
    loop_id = registry.open_loop(mrn="MRN1", control_id="C1", message_at=T0)
    registry.record_result(loop_id, obx11="F", control_id="C2", message_at=T1)
    registry.acknowledge(loop_id, actor="coord1", role="coordinator", control_id="C3")
...
def test_close_requires_an_explicit_final_or_corrected_status(registry, store):
    ...
    close just because its status is not literally 'P'."""
...
def test_close_is_unreachable_with_no_result_at_all(registry):
    """An order nobody resulted must never be closeable."""
    loop_id = registry.open_loop(mrn="MRN1", control_id="C1")
    with pytest.raises(ReferralLoopError):
...
def test_an_orphan_is_not_closeable(registry):
    """Orphans hold results nobody ordered. Closing one would retire a result
...
def test_an_orphan_cannot_be_resulted_out_of_the_orphan_queue(registry):
    """ORPHAN -> RESULTED -> CLOSED would retire a result nobody ordered through
...
def test_reacknowledging_a_closed_loop_is_refused(registry):
    loop_id = registry.open_loop(mrn="MRN1", control_id="C1")
    registry.record_result(loop_id, obx11="F", control_id="C2")
    registry.acknowledge(loop_id, actor="coord1", role="coordinator", control_id="C3")
    with pytest.raises(ReferralLoopError):
...
def test_acknowledge_requires_a_named_actor_and_role(registry):
    """CLOSED attributed to nobody cannot answer 'who closed this'."""
    loop_id = registry.open_loop(mrn="MRN1", control_id="C1")
    registry.record_result(loop_id, obx11="F", control_id="C2")
    with pytest.raises(ReferralLoopError):
...
def test_acknowledgement_records_which_result_was_acknowledged(registry, store):
    ...
    closed = [e for e in store.events_for(loop_id) if e.event_type == "closed"]
    assert closed[-1].detail["ack_result_status"] == "F"
    assert closed[-1].control_id == "C3"
...
def test_a_repeat_result_on_a_closed_loop_does_not_hide_it(registry, store):
    """A final resent after close moves CLOSED -> RESULTED. If it left ack_at
    ...
    assert loop_id in ids, "a result on a closed loop must return it to a worklist"
...
def test_cancel_of_a_closed_loop_is_refused(registry):
    loop_id = registry.open_loop(mrn="MRN1", control_id="C1")
    registry.record_result(loop_id, obx11="F", control_id="C2")
    registry.acknowledge(loop_id, actor="coord1", role="coordinator", control_id="C3")
    with pytest.raises(ReferralLoopError):
...
def test_a_stale_result_is_refused_and_not_applied(registry, store):
    ...
    CLOSED on a superseded read, defeating safety rule 2."""
    loop_id = registry.open_loop(mrn="MRN1", control_id="C1", message_at=T0)
    registry.record_result(loop_id, obx11="F", control_id="C2", message_at=T1)
    registry.acknowledge(loop_id, actor="coord1", role="coordinator", control_id="C3")
    registry.record_result(loop_id, obx11="C", control_id="C4", message_at=T2)
